...
оплатить"). Payments go through a pluggable provider: **mock** or **T‑Bank (Tinkoff)**.
...
`providers/tbank.js` implements the same interface as
...
+ account tokens). **Card data is never stored here — the provider (T‑Bank) holds it.**
...
- Verify webhook signatures (T‑Bank `Token`).
