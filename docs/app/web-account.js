...
(function () {
  ...
  function tbtn() {
    ...
    var price = selectedInterval === 'year' ? (PRICE.priceYearly + ' ' + t('perYear')) : (PRICE.priceMonthly + ' ' + t('perMonth'));
    var label = used ? (t('subscribe') + ' — ' + price) : t('trial');
    return ptoggle() + '<button class="btn primary btn-lg" data-acc="trial">' + label + '</button>'
  ...
  document.addEventListener('click', function (ev) {
    ...
    if (act === 'trial') busy(call('POST', '/v1/billing/start-trial', { interval: selectedInterval })).then(expectRedirect);
