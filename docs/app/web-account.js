...
(function () {
  ...
  var consented = false; // T-Bank requires explicit, user-ticked consent to recurring charges before pay
  ...
  var triedServer = false; // first /v1/status attempt finished (success or not)
  function dropSession() {
    // Token rejected (evicted by the device cap or revoked) — clear the ghost session.
    state.token = null; state.license = null; state.serverEnt = null; state.account = null; persist();
    if (window.DriftlyToast) window.DriftlyToast(t('sessionEnded'), 'warn');
  }
  async function refresh() {
    if (preview() || !state.token) { emit(); return; }
    try {
      var r = await fetch(state.api + '/v1/status', { headers: headers() });
      if (r.status === 401) { state.online = true; triedServer = true; dropSession(); emit(); return; }
      var j = await r.json();
  ...
  async function call(method, p, body, quiet) {
    try {
      var r = await fetch(state.api + p, { method: method, headers: headers(), body: body ? JSON.stringify(body) : undefined });
      if (r.status === 401) { state.online = true; dropSession(); emit(); return { ok: false, error: 'unauthorized' }; }
      var j = await r.json();
      if (j.license) state.license = j.license;
      ...
      // Offline / server unreachable: tell the user instead of failing silently.
      state.online = false; emit();
      if (!quiet && window.DriftlyToast) window.DriftlyToast(t('errOffline'), 'warn');
  ...
  /* ------------------------------- rendering ------------------------------- */
  var L = {
    ru: { preview: 'Демо-режим: сервер лицензий не подключён — доступ открыт.', signin: 'Войдите, чтобы управлять подпиской.', trial: 'Подключить карту — 3 дня бесплатно', subscribe: 'Оформить подписку', trialActive: 'Пробный период', daysLeft: 'дн. осталось', active: 'Подписка активна', renews: 'Продление', inactive: 'Подписка неактивна', pastDue: 'Необходимо оплатить', pastDueDesc: 'Списание не прошло. Оплатите, чтобы продолжить.', errNoRebill: 'Карта не привязана. Нажмите «Изменить карту», чтобы продолжить.', errDeclined: 'Банк отклонил списание. Проверьте карту или привяжите другую («Изменить карту»).', errInit: 'Не удалось создать платёж. Попробуйте позже или обновите карту.', retryFailed: 'Оплатить не удалось', retry: 'Повторить оплату', cancel: 'Отменить подписку', updateCard: 'Изменить карту', activating: 'Активируем подписку… несколько секунд', confirmCancel: 'Отменить подписку? Доступ сохранится до конца оплаченного периода, автосписание не произойдёт.', confirmPlanQ: 'Перейти на тариф', bindFailed: 'Не удалось привязать карту. Попробуйте ещё раз.', payNotConfirmed: 'Платёж не подтверждён. Завершите оплату или попробуйте ещё раз.', trialUsedEnded: 'Пробный период использован — закончился', subWasUntil: 'подписка действовала до', noTrialNote: 'Пробный период уже был использован и повторно не предоставляется — абонентская плата спишется сразу при оформлении.', pwText: 'Подключите карту и получите 3 дня бесплатно. Driftly работает и в браузере, и в десктоп-приложении.', pwTextUsed: 'Пробный период уже использован. Оформите подписку, чтобы продолжить — Driftly работает и в браузере, и в десктоп-приложении.', errOffline: 'Нет соединения с сервером', errSend: 'Не удалось отправить код. Попробуйте ещё раз.', errLocked: 'Слишком много попыток — запросите новый код', offlineNote: 'Нет связи с сервером — показаны сохранённые данные.', sessionEnded: 'Сеанс завершён — войдите снова.', goSub: 'Открыть подписку', codeBad: 'Неверный код', emailEmpty: 'Введите email', emailBad: 'Неверный формат email', codeSent: 'Код отправлен — проверьте почту', sentTo: 'Код отправлен на', resend: 'Отправить ещё раз', changeEmail: 'Изменить email', codeValidFor: 'Код действителен ещё', codeExpired: 'Код истёк — запросите новый', resume: 'Возобновить', accessUntil: 'доступ до', trialCanceled: 'Пробный период отменён', subCanceled: 'Подписка отменена', noRenew: 'продление не произойдёт', monthly: 'Помесячно', yearly: 'За год', perMonth: '₽/мес', perYear: '₽/год', planYearWord: 'годовая', planMonthWord: 'месячная', changePlan: 'Тариф', intervalNote: 'Смена тарифа применится со следующего списания.', planCur: 'Текущий тариф', planUntil: 'активен до', planFrom: 'С этого момента вы перейдёте на тариф', planTrialUntil: 'Пробный период активен до', planTrialThen: 'По его окончании спишется', planConfirmQ: 'Подтвердить смену тарифа?', monthWord: 'в месяц', yearWord: 'в год', consentTrial: 'Я соглашаюсь, что после бесплатного пробного периода (3 дня) с меня будет автоматически списываться {amt} до отмены подписки (рекуррентный платёж).', consentPaid: 'Я соглашаюсь на автоматическое списание {amt} до отмены подписки (рекуррентный платёж).' },
    en: { preview: 'Demo mode: no licensing server — access is open.', signin: 'Sign in to manage your subscription.', trial: 'Add a card — 3 days free', subscribe: 'Subscribe', trialActive: 'Free trial', daysLeft: 'days left', active: 'Subscription active', renews: 'Renews', inactive: 'Subscription inactive', pastDue: 'Payment required', pastDueDesc: 'The charge failed. Pay to continue.', errNoRebill: 'No card on file. Tap “Change card” to continue.', errDeclined: 'The bank declined the charge. Check your card or add another (“Change card”).', errInit: 'Could not start the payment. Try later or update the card.', retryFailed: 'Payment failed', retry: 'Retry payment', cancel: 'Cancel subscription', updateCard: 'Change card', activating: 'Activating your subscription… a few seconds', confirmCancel: 'Cancel subscription? Access stays until the end of the paid period; no auto-charge will happen.', confirmPlanQ: 'Switch to plan', bindFailed: 'Could not link the card. Please try again.', payNotConfirmed: 'The payment was not confirmed. Complete it or try again.', trialUsedEnded: 'Free trial used — ended', subWasUntil: 'subscription was active until', noTrialNote: 'The free trial has already been used and is not granted again — the fee is charged immediately at checkout.', pwText: 'Add a card and get 3 days free. Driftly works in the browser and in the desktop app.', pwTextUsed: 'Your free trial has been used. Subscribe to continue — Driftly works in the browser and in the desktop app.', errOffline: 'No connection to the server', errSend: 'Could not send the code. Try again.', errLocked: 'Too many attempts — request a new code', offlineNote: 'No connection — showing cached data.', sessionEnded: 'Session ended — please sign in again.', goSub: 'Open subscription', codeBad: 'Invalid code', emailEmpty: 'Enter your email', emailBad: 'Invalid email format', codeSent: 'Code sent — check your email', sentTo: 'Code sent to', resend: 'Resend code', changeEmail: 'Change email', codeValidFor: 'Code valid for', codeExpired: 'Code expired — request a new one', resume: 'Resume', accessUntil: 'access until', trialCanceled: 'Trial cancelled', subCanceled: 'Subscription cancelled', noRenew: 'will not renew', monthly: 'Monthly', yearly: 'Yearly', perMonth: '₽/mo', perYear: '₽/yr', planYearWord: 'yearly', planMonthWord: 'monthly', changePlan: 'Plan', intervalNote: 'The plan change applies from your next charge.', planCur: 'Current plan', planUntil: 'is active until', planFrom: 'From then you switch to', planTrialUntil: 'Free trial is active until', planTrialThen: 'When it ends you will be charged', planConfirmQ: 'Confirm the plan change?', monthWord: 'per month', yearWord: 'per year', consentTrial: 'I agree that after the free 3-day trial, {amt} will be charged automatically until I cancel (recurring payment).', consentPaid: 'I agree to an automatic charge of {amt} until I cancel (recurring payment).' },
  };
  ...
  function render() {
    ...
    if (pw) {
      ...
      if (show) {
        ...
        $('pw-text').textContent = e.needsPayment ? t('pastDueDesc') : (state.account && state.account.trialUsed ? t('pwTextUsed') : t('pwText'));
    ...
    if (activating && !(e.reason === 'trial' || e.reason === 'active')) { box.innerHTML = sb('trial', '<i class="spinner"></i>', t('activating'), ''); return; }
    ...
    else if (e.needsPayment) box.innerHTML = sb('bad', '⚠', t('pastDue'), pastDueReason())
    ...
    else box.innerHTML = sb('', '🔓', t('inactive'), endedInfo()) + tbtn();
    // Connectivity is a different problem than "no subscription" — say so explicitly.
    if (triedServer && !state.online) box.innerHTML += '<div class="mode-note">' + t('offlineNote') + '</div>';
  }
  // Human, localized reason for a past_due state — from the last failed charge (account.lastError).
  function pastDueReason() {
    var le = state.account && state.account.lastError;
    var base = le && le.code === 'no_rebill_id' ? t('errNoRebill')
      : le && le.code === 'charge_declined' ? t('errDeclined')
      : le && le.code === 'init_failed' ? t('errInit')
      : t('pastDueDesc');
    var msg = le && le.message ? String(le.message).trim() : '';
  ...
  function endedInfo() {
    var a = state.account; if (!a) return '';
    var parts = [];
    if (a.trialUsed && a.trialEndsAt) parts.push(t('trialUsedEnded') + ' ' + fmt(a.trialEndsAt));
    if (a.currentPeriodEnd && a.currentPeriodEnd < Date.now()) parts.push(t('subWasUntil') + ' ' + fmt(a.currentPeriodEnd));
    return parts.join(' · ');
  }
  function ptoggle() {
  ...
  // "Add a card — 3 days free" only for accounts that still have their one-time free trial;
  // a returning user (trialUsed) subscribes and is charged immediately, so the label reflects that.
  function tbtn() {
    var used = !!(state.account && state.account.trialUsed);
    var yr = selectedInterval === 'year';
    var price = yr ? (PRICE.priceYearly + ' ' + t('perYear')) : (PRICE.priceMonthly + ' ' + t('perMonth'));
    var label = used ? (t('subscribe') + ' — ' + price) : t('trial');
    var amt = (yr ? PRICE.priceYearly : PRICE.priceMonthly) + ' ₽ ' + (yr ? t('yearWord') : t('monthWord'));
    var consentTxt = (used ? t('consentPaid') : t('consentTrial')).replace('{amt}', amt);
    // T-Bank requirement: show amount + periodicity and require an explicit, user-ticked
    // consent before the recurring subscription can be started.
    return ptoggle()
      + '<label class="consent"><input type="checkbox" id="sub-consent"' + (consented ? ' checked' : '') + '><span>' + consentTxt + '</span></label>'
      + '<button class="btn primary btn-lg" data-acc="trial"' + (consented ? '' : ' disabled') + '>' + label + '</button>'
  ...
  document.addEventListener('click', function (ev) {
    ...
    if (a.dataset.interval) {
      ...
      } else { selectedInterval = a.dataset.interval; consented = false; render(); }
      return;
    }
    var act = a.dataset.acc;
    // Disable the pressed button until the request settles — double clicks on a slow
    // network would otherwise open two payment sessions.
    function busy(p) { a.disabled = true; a.classList.add('busy'); return p.then(function (j) { a.disabled = false; a.classList.remove('busy'); return j; }); }
    // The provider answered but gave no payment URL (and nothing got activated) — say so.
    function expectRedirect(j) {
      var url = j && j.result && j.result.redirectUrl;
      if (url) { window.location.href = url; return; } // T-Bank: card form + 3-D Secure
      var st = j && j.account && j.account.status;
      if (j && j.error !== 'offline' && st !== 'trialing' && st !== 'active' && window.DriftlyToast) window.DriftlyToast(t('bindFailed'), 'warn');
    }
    if (act === 'trial') { if (!consented) return; busy(call('POST', '/v1/billing/start-trial', { interval: selectedInterval })).then(expectRedirect); }
    else if (act === 'attach-card') busy(call('POST', '/v1/billing/attach-card')).then(expectRedirect);
    else if (act === 'retry') busy(call('POST', '/v1/billing/retry')).then(function () { if (entitlement().needsPayment && window.DriftlyToast) window.DriftlyToast(t('retryFailed') + ': ' + pastDueReason(), 'warn'); });
  ...
  document.addEventListener('change', function (ev) {
    if (ev.target && ev.target.id === 'sub-consent') {
      ...
      var btn = document.querySelector('[data-acc="trial"]');
  ...
  async function sendCode(email) {
    ...
    notify(r && r.error === 'offline' ? t('errOffline') : t('errSend'), 'warn');
    return false;
  }
  ...
  if ($('btn-getcode')) $('btn-getcode').addEventListener('click', function () {
    ...
    var b = $('btn-getcode'); b.disabled = true; b.classList.add('busy');
    sendCode(email).then(function () { b.disabled = false; b.classList.remove('busy'); });
  });
  ...
  async function verifyCode() {
    ...
    verifying = false;
    if (r && r.ok) { clearCode(); return; }
    if (r && r.error === 'code_expired') { localStorage.setItem('driftly.codeExp', String(Date.now() - 1)); runCodeTimer(); OTP.clear(); return; }
    var msg = r && r.error === 'offline' ? t('errOffline') : (r && r.error === 'too_many_attempts' ? t('errLocked') : t('codeBad'));
    notify(msg, 'warn'); OTP.clear(); OTP.focusFirst();
    // notify() replaced the countdown note — restore it so «Отправить ещё раз» stays reachable.
    setTimeout(function () { if (localStorage.getItem('driftly.codeEmail') && !codeExpired()) runCodeTimer(); }, 2200);
  }
  ...
  if ($('btn-signout')) $('btn-signout').addEventListener('click', function () {
    ...
    state.token = null; state.license = null; state.serverEnt = null; state.account = null; persist(); emit();
  });
  ...
  function confirmCardBinding() {
    ...
    var tries = 0;
    var run = function () {
      call('POST', '/v1/billing/confirm-card', null, true).then(function () {
        var e = entitlement();
        if (e.reason === 'trial' || e.reason === 'active') { activating = false; render(); return; }
        // Fast for the first ~20 s (GetState confirms an actually-paid payment almost at
        // once), then ease off briefly. Give up after ~60 s: an unconfirmed payment won't
        // confirm itself by polling — return the normal UI so the user can retry.
        if (++tries >= 20) { activating = false; if (window.DriftlyToast) window.DriftlyToast(t('payNotConfirmed'), 'warn'); render(); return; }
        setTimeout(run, tries < 10 ? 2000 : 4000);
      });
    };
    // Small initial delay so the payment has settled before the first GetState.
    setTimeout(run, 900);
  }
  ...
  if (/[?&]paid=1\b/.test(location.search)) {
    ...
    // A lingering server-side 'pending' (abandoned/failed payment form) must NOT lock the
    // UI in an "activating…" state — the user needs the subscribe button to try again.
    // One quiet confirm attempt covers "paid earlier but the webhook never arrived".
    window.addEventListener('driftly-access-changed', function once() {
      window.removeEventListener('driftly-access-changed', once);
      if (state.account && state.account.status === 'pending') call('POST', '/v1/billing/confirm-card', null, true);
    });
  }
}());
