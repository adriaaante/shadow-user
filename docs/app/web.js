...
(function () {
  ...
  const M = {
    ...
    csv() { const rows = [['minute_iso', 'generator_enabled', 'syn_move', 'syn_click', 'syn_scroll', 'real_move', 'real_click', 'real_scroll', 'synthetic_score', 'real_score']]; [...this.buckets.values()].sort((a, b) => a.ts - b.ts).forEach((b) => rows.push([new Date(b.ts).toISOString(), b.genEnabled ? 1 : 0, b.synthetic.move, b.synthetic.click, b.synthetic.scroll, b.real.move, b.real.click, b.real.scroll, score(b.synthetic), score(b.real)])); return rows.map((r) => r.join(',')).join('\n'); },
